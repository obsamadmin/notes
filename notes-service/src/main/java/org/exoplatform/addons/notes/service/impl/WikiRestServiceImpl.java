...
import org.exoplatform.addons.notes.utils.WikiHTMLSanitizer;
...
import org.exoplatform.addons.notes.mow.api.EmotionIcon;
import org.exoplatform.addons.notes.mow.api.Wiki;
import org.exoplatform.addons.notes.mow.api.WikiType;
import org.exoplatform.addons.notes.service.Relations;
import org.exoplatform.addons.notes.service.WikiPageParams;
import org.exoplatform.addons.notes.service.WikiService;
import org.exoplatform.addons.notes.service.image.ResizeImageService;
import org.exoplatform.addons.notes.service.related.JsonRelatedData;
import org.exoplatform.addons.notes.service.related.RelatedUtil;
import org.exoplatform.addons.notes.service.rest.model.Attachment;
import org.exoplatform.addons.notes.service.rest.model.Attachments;
import org.exoplatform.addons.notes.service.rest.model.Link;
import org.exoplatform.addons.notes.service.rest.model.ObjectFactory;
import org.exoplatform.addons.notes.service.rest.model.PageSummary;
import org.exoplatform.addons.notes.service.rest.model.Pages;
import org.exoplatform.addons.notes.service.rest.model.Space;
import org.exoplatform.addons.notes.service.rest.model.Spaces;
import org.exoplatform.addons.notes.service.search.SearchResult;
import org.exoplatform.addons.notes.service.search.SearchResultType;
import org.exoplatform.addons.notes.service.search.TitleSearchResult;
import org.exoplatform.addons.notes.service.search.WikiSearchData;
import org.exoplatform.addons.notes.tree.JsonNodeData;
import org.exoplatform.addons.notes.tree.TreeNode;
import org.exoplatform.addons.notes.tree.TreeNode.TREETYPE;
import org.exoplatform.addons.notes.tree.WikiTreeNode;
...
import org.exoplatform.addons.notes.utils.WikiNameValidator;
...
public class WikiRestServiceImpl implements ResourceContainer {
    ...
    if (isMultipart) {
      ...
      try {
        ...
        for (FileItem fileItem : items) {
          ...
          org.exoplatform.addons.notes.mow.api.Page page = wikiService.getExsitedOrNewDraftPageById(wikiType, wikiOwner, pageId);
          org.exoplatform.addons.notes.mow.api.Attachment attachment = new org.exoplatform.addons.notes.mow.api.Attachment();
  ...
  public Response getTreeData(@PathParam("type") String type,
    ...
    try {
      ...
      if (currentPath != null){
        ...
        org.exoplatform.addons.notes.mow.api.Page currentPage = wikiService.getPageOfWikiByName(currentPageParam.getType(), currentPageParam.getOwner(), currentPageParam.getPageName());
      ...
      org.exoplatform.addons.notes.mow.api.Page page = wikiService.getPageOfWikiByName(pageParam.getType(), pageParam.getOwner(), pageParam.getPageName());
      if (page == null) {
  ...
  public Response getRelated(@QueryParam(TreeNode.PATH) String path) {
    ...
    try {
      ...
      org.exoplatform.addons.notes.mow.api.Page page = wikiService.getPageOfWikiByName(params.getType(), params.getOwner(), params.getPageName());
      if (page != null) {
        List<org.exoplatform.addons.notes.mow.api.Page> relatedPages = wikiService.getRelatedPagesOfPage(page);
  ...
  public Spaces getSpaces(@Context UriInfo uriInfo,
    ...
    try {
      ...
      for (String spaceName : spaceNames) {
        org.exoplatform.addons.notes.mow.api.Page page = wikiService.getPageOfWikiByName(wikiType, spaceName, WikiConstants.WIKI_HOME_NAME);
  ...
  public Spaces getLastVisitedSpaces(@Context UriInfo uriInfo,
    ...
    try {
      ...
      for (Object space : lastVisitedSpaces) {
        ...
        org.exoplatform.addons.notes.mow.api.Page page = wikiService.getPageOfWikiByName(wiki.getType(), wiki.getOwner(), WikiConstants.WIKI_HOME_NAME);
  ...
  public Space getSpace(@Context UriInfo uriInfo,
                        @PathParam("wikiType") String wikiType,
                        @PathParam("wikiOwner") String wikiOwner) {
    org.exoplatform.addons.notes.mow.api.Page page;
    try {
  ...
  public Pages getPages(@Context UriInfo uriInfo,
    ...
    try {
      ...
      if (isWikiHome) {
        ...
        List<org.exoplatform.addons.notes.mow.api.Page> childrenPages = wikiService.getChildrenPageOf(page);
        for (org.exoplatform.addons.notes.mow.api.Page childPage : childrenPages) {
  ...
  @Produces("application/xml")
  @RolesAllowed("users")
  public org.exoplatform.addons.notes.service.rest.model.Page getPage(@Context UriInfo uriInfo,
                                                              @PathParam("wikiType") String wikiType,
                                                              @PathParam("wikiOwner") String wikiOwner,
                                                              @PathParam("pageId") String pageId) {
    org.exoplatform.addons.notes.mow.api.Page page;
  ...
  public Attachments getAttachments(@Context UriInfo uriInfo,
    ...
    try {
      page = wikiService.getPageOfWikiByName(wikiType, wikiOwner, pageId);
      List<org.exoplatform.addons.notes.mow.api.Attachment> pageAttachments = wikiService.getAttachmentsOfPage(page);
      for (org.exoplatform.addons.notes.mow.api.Attachment pageAttachment : pageAttachments) {
  ...
  public Response searchData(@Context UriInfo uriInfo,
    ...
    try {
      ...
      for (SearchResult searchResult : results) {
        org.exoplatform.addons.notes.mow.api.Page page = wikiService.getPageOfWikiByName(searchResult.getWikiType(), searchResult.getWikiOwner(), searchResult.getPageName());
        if(page != null) {
          if (SearchResultType.ATTACHMENT.equals(searchResult.getType())) {
            org.exoplatform.addons.notes.mow.api.Attachment attachment = wikiService.getAttachmentOfPageByName(searchResult.getAttachmentName(), page);
  ...
  public Response getAttachment(@Context UriInfo uriInfo,
    ...
    InputStream result;
    try {
      org.exoplatform.addons.notes.mow.api.Page page = wikiService.getPageOfWikiByName(wikiType, wikiOwner, pageId);
      if (page == null) {
      ...
      org.exoplatform.addons.notes.mow.api.Attachment attachment = wikiService.getAttachmentOfPageByName(attachmentId, page, true);
  ...
  public Space createSpace(ObjectFactory objectFactory,
                           ...
                           String wikiName,
                           String spaceName,
                           org.exoplatform.addons.notes.mow.api.Page home) {
  ...
  public org.exoplatform.addons.notes.service.rest.model.Page createPage(ObjectFactory objectFactory,
                                                                 URI baseUri,
                                                                 URI self,
                                                                 org.exoplatform.addons.notes.mow.api.Page doc) throws Exception {
    org.exoplatform.addons.notes.service.rest.model.Page page = objectFactory.createPage();
  ...
  public PageSummary createPageSummary(ObjectFactory objectFactory, URI baseUri, org.exoplatform.addons.notes.mow.api.Page page) throws IllegalArgumentException, UriBuilderException, Exception {
  ...
  public Attachment createAttachment(ObjectFactory objectFactory,
                                     URI baseUri,
                                     org.exoplatform.addons.notes.mow.api.Attachment pageAttachment,
                                     org.exoplatform.addons.notes.mow.api.Page page,
  ...
  private void fillPageSummary(PageSummary pageSummary,
                                      ...
                                      URI baseUri,
                                      org.exoplatform.addons.notes.mow.api.Page page) throws IllegalArgumentException, UriBuilderException, Exception {
    Wiki wiki = wikiService.getWikiByTypeAndOwner(page.getWikiType(), page.getWikiOwner());
    ...
    org.exoplatform.addons.notes.mow.api.Page parent = wikiService.getParentPageOf(page);
    ...
    List<org.exoplatform.addons.notes.mow.api.Attachment> attachments = wikiService.getAttachmentsOfPage(page);
  ...
  public Response saveDraft(@QueryParam("wikiType") String wikiType,
    ...
    String pageId = null;
    try {
      if ("__anonim".equals(org.exoplatform.addons.notes.utils.Utils.getCurrentUser())) {
