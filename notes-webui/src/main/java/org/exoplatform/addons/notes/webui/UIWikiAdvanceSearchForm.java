...
import org.exoplatform.addons.notes.webui.core.UIAdvancePageIterator;
...
public class UIWikiAdvanceSearchForm extends UIForm {
  ...
  public UIWikiAdvanceSearchForm() throws Exception {
    ...
    selectSpaces.setCurrentSpaceName(org.exoplatform.addons.notes.commons.Utils.upperFirstCharacter(
           org.exoplatform.addons.notes.commons.Utils.getCurrentSpaceName()));
  ...
  public void resetWikiSpaces() throws Exception {
    getChild(UISpacesSwitcher.class).setCurrentSpaceName(
           org.exoplatform.addons.notes.commons.Utils.upperFirstCharacter(org.exoplatform.addons.notes.commons.Utils.getCurrentSpaceName()));
  ...
  private WikiSearchData createSearchData() {
    ...
    if (path != null && path.startsWith(org.exoplatform.addons.notes.commons.Utils.SLASH)) {
      path = path.substring(1);
    }
    String wikiType = null;
    String wikiOwner = null;
    if (!StringUtils.isEmpty(path)) {
      String[] arrayParams = path.split(org.exoplatform.addons.notes.commons.Utils.SLASH);
      if (arrayParams.length >= 1) {
        wikiType = arrayParams[0];
        if (arrayParams.length >= 2)
          wikiOwner = StringUtils.replace(path, wikiType + org.exoplatform.addons.notes.commons.Utils.SLASH, StringUtils.EMPTY);
      }
    }
  ...
  private String getDefaultSelectWikiValue() throws Exception {
    WikiPageParams currentParams = org.exoplatform.addons.notes.commons.Utils.getCurrentWikiPageParams();
