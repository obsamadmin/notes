...
public class UIWikiBreadCrumb extends UIContainer {
  ...
  public String getWikiName() throws Exception {
    ...
    if (params != null) {
      ...
      return org.exoplatform.addons.notes.commons.Utils.getSpaceName(wikiService.getWikiByTypeAndOwner(wikiType, wikiOwner));
    }
    return null;
  }
  ...
  public static class SwitchSpaceActionListener extends EventListener<UIWikiBreadCrumb> {
    public void execute(Event<UIWikiBreadCrumb> event) throws Exception {
      ...
      String link = org.exoplatform.addons.notes.commons.Utils.getURLFromParams(new WikiPageParams(wiki.getType(), wiki.getOwner(), wikiHome.getName()));
      org.exoplatform.addons.notes.commons.Utils.ajaxRedirect(event, link);
    }
  }
}
